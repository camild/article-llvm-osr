...
To explain how \tinyvm\ works, we consider the toy example of \myfigure\ref{fi:isord-example}. Function {\tt isord} checks whether an array of numbers is sorted according to some ordering given by a comparator. We explore a scenario where control is passed to a faster version with comparator inlining if the number of iterations exceeds a certain threshold. We use deferred compilation by instrumenting {\tt isord} in \tinyvm\ with an open OSR, as shown in \myfigure\ref{fig:isordfrom}.
