...
function to resume the execution (as \fosrto\ in \osrkit) is likely to yield better performance. \osrkit\ addresses a few limitations of McOSR, including: 1) only one version of a function can be active in memory; 2) there is no provision for resolved OSR points or compensation code; 3) OSR points can be inserted at loop headers only; 4) MCJIT does not allow to modify a JIT-ted IR function, and is this incompatible with McOSR's design.