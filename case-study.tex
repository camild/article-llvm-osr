...
A new function $f_{opt}$ is then generated by cloning the IIR representation $f^{IIR}$ of $f$ into $f^{IIR}_{opt}$ and replacing all the \feval\ calls in the same group of the instrumented one with direct calls to $g$.

As a next step, the optimizer asks the IIR compiler to generate IR code for $f^{IIR}_{opt}$, also saving a copy of the variable map when compiling the direct call corresponding to the \feval\ instruction that triggered the OSR transition. This variable map is compared against the variable map stored during the lowering phase for $f$ 
