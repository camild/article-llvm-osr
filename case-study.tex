...
When the IIR compiler processes an annotated \feval\ instruction, it keeps track of the current variable map (i.e., a map between IIR and IR objects), the {\tt llvm::BasicBlock*} created for the \feval\ and the {\tt llvm::Value*} object for the first argument of the \feval. The last two objects are used by the inserter component as basic block and {\tt val} argument for the open OSR stub, which invokes the optimizer component we are about to present.
