...
\noindent \tinyvm\ will {\tt UPDATE} the function in the following way: an {\tt ALWAYS}-true OSR condition is verified before executing instruction {\tt \%4}, firing an {\tt OPEN} OSR transition to the {\tt DYN\_INLINE} code generator that will inline any indirect function call to the function pointer {\tt \%c}. We choose {\tt \%4} as location for the OSR as it is the first non-$\phi$ instruction in the loop body, and we hint the LLVM back-end through profiling metadata that the OSR firing is {\tt 100}\%-likely.
